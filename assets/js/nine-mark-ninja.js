// 9-Mark Ninja – multi-scenario core logic

(function () {
  const scenarios = window.NINE_MARK_SCENARIOS || {};
  ...
  const maxStage = 4;
  let currentScenarioKey = "storyboard";
  let scenario = scenarios[currentScenarioKey];
  ...
  function updateScenarioHeader() {
    ...
    if (scenarioQuestionEl) scenarioQuestionEl.innerHTML = scenario.questionText;
    if (scenarioImageEl) {
      scenarioImageEl.src = scenario.imageSrc;
      scenarioImageEl.alt = scenario.title;
    }
  }
  ...
  function renderStage() {
    ...
    if (currentStage === 1) renderStage1();
  ...
  function handleStage1Check() {
    ...
    if (scenario.focusOptions[selectedIndex].correct) {
      feedbackEl.innerHTML =
        "Great – you have correctly identified that you must <strong>discuss the suitability of the " +
        scenario.docLabel +
        "</strong> for the " +
        scenario.userRoleLabel +
        " by suggesting improvements and explaining how they help.";
      score = 10;
    } else {
      feedbackEl.innerHTML =
        "Not quite. A 9-mark answer must focus on how suitable the " +
        scenario.docLabel +
        " is for the " +
        scenario.userRoleLabel +
        " and suggest improvements with clear explanations, not just personal opinion or redesigning the whole product.";
      score = 4;
    }
  ...
  function renderStage2() {
    ...
    stageContainer.innerHTML = `
      ...
      <p>Click all the suggestions that would genuinely help the <strong>${scenario.userRoleLabel}</strong> use this ${scenario.docLabel}. Avoid weaker or irrelevant ideas.</p>
  ...
  function handleStage2Check() {
    ...
    if (weakPicked > 0) {
      message += `You also chose ${weakPicked} weaker/irrelevant ideas – try to focus only on improvements that help the ${scenario.userRoleLabel} use the ${scenario.docLabel}.`;
    } else if (strongPicked === totalStrong) {
      message += "Excellent – you have identified all the high-value improvements.";
    } else {
      message +=
        "Look again at the document and think about what information the " +
        scenario.userRoleLabel +
        " really needs.";
    }
  ...
  function renderStage3() {
    ...
    stageContainer.innerHTML = `
      ...
      <p>For each question, choose the explanation that clearly links the suggestion to how it helps the ${scenario.userRoleLabel}.</p>
      ${questionsHtml}
  ...
  function handleStage3Check() {
    ...
    if (correctCount === total) {
      feedbackEl.textContent =
        "Perfect explanations – you are clearly linking each suggestion to how it helps the " +
        scenario.userRoleLabel +
        ".";
  ...
  function handleStage4Check() {
    ...
    if (percentage >= 75 && weakCount === 0) {
      ...
      bandText =
        "This is closer to Level 1: try to focus less on personal opinion and more on specific improvements plus how they help the " +
        scenario.userRoleLabel +
        ".";
    }
  ...
  function updateHUD() {
    ...
    const percentage = Math.max(
      0,
      Math.min(100, Math.round((total / 62) * 100))
    );
  ...
  function updateNavButtons() {
    ...
    nextStageBtn.style.display =
      currentStage >= maxStage ? "none" : "inline-block";
  }

  // ---------- Navigation & scenario select ----------
  ...
  if (scenarios[currentScenarioKey]) {
    switchScenario(currentScenarioKey);
  }
})();
