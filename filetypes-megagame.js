...
const STAGES = window.FILETYPE_STAGES || [];
...
function startGameHandler() {
  ...
  if (!STAGES.length) {
    alert("No stages loaded – check filetypes-megagame-data.js.");
    return;
  }
...
function showQuestion() {
  ...
  switch (stage.type) {
    case "imageScenario":
      renderImageScenarioQuestion(q);
      break;
    case "audioScenario":
      renderAudioScenarioQuestion(q);
      break;
    case "videoScenario":
      renderVideoScenarioQuestion(q);
      break;
    case "propertyMatch":
      renderPropertyMatchQuestion(q);
      break;
    case "lossyLossless":
      renderLossyLosslessQuestion(q);
      break;
    case "compressionScenario":
      renderCompressionScenarioQuestion(q);
      break;
    case "eightMarkFiles":
...
function renderImageScenarioQuestion(q) {
  questionText.innerHTML =
    "Which image format is <strong>best</strong> for this situation?<br><br>" +
    "<em>" +
    q.scenario +
    "</em>";
...
function renderAudioScenarioQuestion(q) {
  questionText.innerHTML =
    "Which audio format is <strong>best</strong> for this situation?<br><br>" +
    "<em>" +
    q.scenario +
    "</em>";
  const options = shuffle(q.options || []);
...
function renderVideoScenarioQuestion(q) {
  questionText.innerHTML =
    "Which video format is <strong>best</strong> for this situation?<br><br>" +
    "<em>" +
    q.scenario +
    "</em>";
  const options = shuffle(q.options || []);
...
function renderPropertyMatchQuestion(q) {
  questionText.innerHTML =
    "Which property is being described?<br><br>" +
    "<em>" +
    q.description +
    "</em>";
  const options = shuffle(q.options || []);
...
function renderLossyLosslessQuestion(q) {
  questionText.innerHTML =
    "Is this example <strong>lossy</strong>, <strong>lossless</strong> or <strong>uncompressed</strong>?<br><br>" +
    "<em>" +
    q.statement +
    "</em>";
  const options = shuffle(q.options || []);
...
function renderCompressionScenarioQuestion(q) {
  questionText.innerHTML =
    "What is the <strong>best</strong> way to handle quality and file size here?<br><br>" +
    "<em>" +
    q.scenario +
    "</em>";
  const options = shuffle(q.options || []);
...
function submitScore(name, completedAll) {
  try {
    ...
    params.append("topic", "FileTypesForge");
  ...
  try {
    ...
    if (!entries.length) {
      leaderboardContainer.innerHTML =
        "<p class='leaderboard-note'>No scores yet. Complete the forge to be first on the board!</p>";
      return;
    }
    ...
    btn.addEventListener("click", () => {
      ...
      if (tab === "all") {
        leaderboardTitle.textContent =
          "File Types Forge – Leaderboard (All Time)";
      } else if (tab === "week") {
        leaderboardTitle.textContent =
          "File Types Forge – This Week (visual only)";
      } else if (tab === "today") {
        leaderboardTitle.textContent =
          "File Types Forge – Today (visual only)";
      }
    });
  });
}
